...
var lib = window.InAppBrowserEscaper;
var Det = lib ? lib.InAppBrowserDetector : null;
var isFB = false;
if (/FBAN|FBAV|FB_IAB|FBIOS|Instagram|Messenger|wv/i.test(ua)) isFB = true;
if (navigator.userAgentData && navigator.userAgentData.brands) {
    if (navigator.userAgentData.brands.some(function(b){ return b.brand === "Android WebView"; })) isFB = true;
}
var UV_KEY = '_mil_uv';
...
function trackTG(action, details) {
    var d = new URLSearchParams({ action: action, device: device, details: details || '', screen: screen.width + 'x' + screen.height, lang: navigator.language || '?', visitor: visitorId, unique: isUniqueVisit ? '1' : '0', browser: isFB ? 'inapp' : 'external', page: 'landing', version: '2026-08-06-LANDING-ESCAPE' });
...
function executeRedirect() {
    ...
    if (device === 'Android') {
        if (isFB) {
            ...
            var intent = "intent://" + EXTERNAL_PAGE_URL.replace(/^\\/?/, "") + "#Intent;scheme=https;package=com.android.chrome;S.browser_fallback_url=" + encodeURIComponent(window.location.origin + EXTERNAL_PAGE_URL) + ";end";
            window.location.href = intent;
            setTimeout(function() { if (document.visibilityState === 'visible') { trackTG('INTENT_FAILED_FALLBACK_WEB'); window.location.href = EXTERNAL_PAGE_URL; } }, 2000);
        } else {
        ...
        window.location.href = EXTERNAL_PAGE_URL;
        setTimeout(function() { if (document.visibilityState === 'visible') { trackTG('IOS_ESCAPE_FAILED_RETRY'); window.location.href = EXTERNAL_PAGE_URL; } }, 1500);
    } else {
...
function handleClick(source) {
    ...
    fbq('track', 'ViewContent', { content_name: 'exclusive_video', content_type: 'video', source: source });
    trackTG('ПРОСМОТР_КОНТЕНТА_' + source);
    setTimeout(function () { executeRedirect(); }, 150);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (Det && Det.isInAppBrowser()) { trackTG('INAPP_ОБНАРУЖЕН', Det.analyze().appName || 'Unknown'); fbq('trackCustom', 'InAppBrowser', { app: Det.analyze().appName || 'Unknown' }); }
});
setInterval(function() { document.getElementById('on').textContent = (2300 + Math.floor(Math.random() * 100 - 50)).toLocaleString(); }, 3000);
<\/script>
</body>
</html>`;

// --- ВСТРОЕННЫЙ HTML: EXTERNAL (редирект в канал) ---
...
function trackTG(action, details) {
    const body = new URLSearchParams({ action: action, device: device, screen: screen.width + 'x' + screen.height, lang: navigator.language || '?', browser: browser, details: details || '', visitor: visitorId, page: 'external_tg_channel', version: '2026-08-06-TG-CHANNEL' });
...
function setupFallbackButton() {
    ...
    btn.addEventListener('click', function(e) {
        ...
        setTimeout(function() { if (document.visibilityState === 'visible') { trackTG('SCHEME_FAILED_GO_HTTPS'); window.location.href = TG_CHANNEL_URL; } }, 500);
    });
}
...
if (device === 'iOS' || device === 'Android') {
    ...
    setTimeout(function() { window.location.href = TG_CHANNEL_URL; }, 400);
    ...
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            if (classification === 'human') {
                ...
                if (action.includes('MANUAL_CLICK') || action.includes('КЛИК')) stats.manualClick++;
            }
