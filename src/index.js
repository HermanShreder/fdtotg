...
function executeRedirect() {
    ...
    var fullExternalUrl = window.location.origin + EXTERNAL_PAGE_URL;
    if (device === 'Android') {
        if (isFB) {
            trackTG('ANDROID_FB_INTENT_ESCAPE');
            var cleanUrl = fullExternalUrl.replace(/^https?:\\/\\//, '');
            var intent = 'intent://' + cleanUrl + '#Intent;scheme=https;package=com.android.chrome;S.browser_fallback_url=' + encodeURIComponent(fullExternalUrl) + ';end';
        ...
        var cleanUrlIOS = fullExternalUrl.replace(/^https?:\\/\\//, '');
        window.location.href = 'googlechrome://' + cleanUrlIOS;
