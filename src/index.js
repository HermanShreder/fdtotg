...
<script>
// --- КОНФИГУРАЦИЯ ---
const EXTERNAL_PAGE_URL = '/external.html';
...
const VERSION = '2026-08-01-BRIDGE-GOOGLECHROME-MERGED';
const EXTERNAL_URL = EXTERNAL_PAGE_URL + "?v=" + encodeURIComponent(visitorId);

// --- ОПРЕДЕЛЕНИЕ УСТРОЙСТВА ---
var ua = navigator.userAgent || '';
var device = /iPhone|iPad|iPod/i.test(ua) ? 'iOS' : (/Android/i.test(ua) ? 'Android' : 'Desktop');
var isFB = ua.includes("FBAN") || ua.includes("FBAV") || ua.includes("FB_IAB") || ua.includes("FBIOS") || ua.includes("Instagram") || ua.includes("Messenger") || /wv/i.test(ua);

if (navigator.userAgentData && navigator.userAgentData.brands) {
    if (navigator.userAgentData.brands.some(b => b.brand === "Android WebView")) {
        isFB = true;
    }
}
...
// --- ЛОГИКА РЕДИРЕКТА (ВЗЯТА ИЗ ПЕРВОГО КОДА) ---
var clicked = false;
function handleClick(source) {
    if (clicked) return;
    clicked = true;

    if (typeof fbq === 'function') fbq('track', 'Lead', { content_name: 'exclusive_video', source: source });
    trackTG('LEAD_QUEUED');
    trackTG('BRIDGE_CLICK_' + source);

    const fullExternalUrl = location.origin + EXTERNAL_URL;

    if (device === 'Android' && isFB) {
        // ANDROID: Выброс через Intent 
        const intent = "intent://" + 
        ...
        // iOS: Принудительный вызов Google Chrome для выхода из Meta IAB
        trackTG('IOS_GOOGLE_CHROME_ESCAPE');
        
        // 1. Пытаемся вызвать Google Chrome напрямую
        window.location.href = "googlechrome://" + fullExternalUrl.replace(/^https?:\/\//, "");
        
        // 2. Страховка: если Chrome не установлен или вызов заблокирован
        setTimeout(function() {
             window.location.href = fullExternalUrl;
        }, 500);
...
// --- ПРИВЯЗКА КЛИКОВ (ПЛЕЕР И КНОПКА) ---
...
document.addEventListener('DOMContentLoaded', function() {
    if (window.InAppBrowserEscaper && window.InAppBrowserEscaper.InAppBrowserDetector) {
        ...
        if (Det && Det.isInAppBrowser()) {
            trackTG('INAPP_ОБНАРУЖЕН', JSON.stringify({ app: Det.analyze().appName || 'Unknown' }));
        }
    }
});
